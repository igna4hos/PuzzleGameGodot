...
@onready var center_root = $Node2D/SubViewportContainer/SubViewport/CenterRoot
@onready var left_root = $Node2D2/SubViewportContainer/SubViewport/Node3D
@onready var right_root = $Node2D3/SubViewportContainer/SubViewport/Node3D
...
var current_index = 0
var current_center: Node3D = null
...
func try_attach_part(part_id: int) -> void:
	if current_center and current_center.try_attach(part_id):
		if current_center.is_complete():
			current_index += 1
...
func load_center_model() -> void:
	if current_center:
		current_center.queue_free()
	var data = center_models[current_index]
	var scene = load(data["scene"]).instantiate()
	center_root.add_child(scene)
	current_center = scene
	current_center.left_id = data["left_id"]
	current_center.right_id = data["right_id"]
